...
export default function GrantIndicatorsTab({ grantId }: { grantId: string }) {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("grant_indicators" as any).delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["grant_indicators", grantId] }); setDeleteItem(null); toast({ title: "Indicateur supprimé" }); },
    onError: (e) => toast({ title: "Erreur", description: (e as Error).message, variant: "destructive" }),
  });

  const grouped = (indicators ?? []).reduce((acc: Record<string, any[]>, ind: any) => {
            ...
            return (
              <div key={cat} className="bg-card border border-border rounded-xl overflow-hidden">
                <div className="px-4 py-2.5 border-b border-border bg-secondary/50">
                  <span className="text-xs font-bold text-foreground">{categoryLabels[cat] ?? cat}</span>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-[12.5px]">
                    ...
                    <tbody>
                      {items.map((ind: any) => {
                        const pct = (ind.target_value ?? 0) > 0 ? Math.round(((ind.current_value ?? 0) / ind.target_value) * 100) : 0;
                        return (
      ...
      <AlertDialog open={!!deleteItem} onOpenChange={(o) => { if (!o) setDeleteItem(null); }}>
        <AlertDialogContent className="bg-card border-border">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>L'indicateur « {deleteItem?.name} » sera définitivement supprimé.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        current_value: parseFloat(currentValue) || 0, data_source: dataSource || null,
        frequency, responsible: responsible || null,
      };
      if (isEdit) {
        const { error } = await supabase.from("grant_indicators" as any).update(payload).eq("id", editData.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("grant_indicators" as any).insert(payload);
        if (error) throw error;
      }
    },
