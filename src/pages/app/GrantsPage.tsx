  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div className="grid grid-cols-3 gap-4">
        {isLoading ? (
          Array.from({ length: 3 }).map((_, i) => <Skeleton key={i} className="h-[240px] rounded-xl" />)
        ) : grants?.length === 0 ? (
          <div className="col-span-3 text-center text-text-secondary py-12 text-sm">Aucun grant pour le moment</div>
        ) : (
          grants?.map((g) => {
              ...
              <div key={g.id} className="bg-card border border-border rounded-xl overflow-hidden hover:border-border/80 hover:-translate-y-0.5 transition-all cursor-pointer">
                <div className="px-4 py-3.5 border-b border-border flex items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="text-[11px] text-text-secondary mt-0.5">{g.organization ?? "—"}</div>
                  </div>
                  <Pill color={st.color}>{st.label}</Pill>
                </div>
                <div className="px-4 py-3.5">
                  {[
                    { label: "Montant total", val: formatXOF(g.amount_total) },
                    { label: "Décaissé", val: formatXOF(g.amount_disbursed ?? 0) },
                    { label: "Utilisation", val: `${pct}%` },
                  ].map((row) => (
                ...
                <div className="px-4 py-2.5 bg-surface-2 border-t border-border">
                  <div className="h-1.5 bg-surface-3 rounded-full overflow-hidden">
                    <div className="h-full rounded-full bg-gh-blue transition-all" style={{ width: `${pct}%` }} />
                  </div>
                </div>
              </div>
            );
